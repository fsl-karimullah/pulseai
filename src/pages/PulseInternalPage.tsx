...
import { Database, BarChart3, Bell, ArrowRight, Loader2, Sparkles, AlertCircle, CheckCircle2 } from 'lucide-react';

const PulseInternalPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('waitlist_internal').insert([{ email }]);
      ...
      setToast({
        show: true,
        message: 'Success! You have been successfully added to the PulseInternal waitlist.',
        type: 'success',
      });
      setEmail('');
    } catch (err: any) {
      ...
      if (err.message?.includes('relation "waitlist_internal" does not exist')) {
        setToast({
          show: true,
          message: 'Demomode: Waitlist saved locally! (Table "waitlist_internal" not yet created in Supabase)',
          type: 'success',
        });
        setEmail('');
      } else {
      ...
      <div className="relative z-10 max-w-4xl mx-auto flex flex-col justify-between h-full">
        <div>
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight leading-tight mb-4">
            PulseInternal — <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-400 to-emerald-500">Your AI Business Data Analyst</span>
          </h1>
          <p className="text-slate-400 text-base sm:text-lg max-w-2xl leading-relaxed mb-12">
            Stop guessing. Start asking. Talk to your business database as easily as chatting on WhatsApp. Connect your data and query operations instantly.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            ...
            <div className="bg-slate-950/40 backdrop-blur-md border border-white/5 hover:border-emerald-500/20 p-6 rounded-2xl transition-all duration-300 group hover:-translate-y-1">
              ...
              <h3 className="text-lg font-bold text-white mb-2">Direct Database Connection</h3>
              <p className="text-slate-400 text-xs sm:text-sm leading-relaxed">
                Securely connect PulseAI to your sales, inventory, or financial data. Strictly confidential and non-public.
              </p>
            </div>
            ...
            <div className="bg-slate-950/40 backdrop-blur-md border border-white/5 hover:border-emerald-500/20 p-6 rounded-2xl transition-all duration-300 group hover:-translate-y-1">
              ...
              <h3 className="text-lg font-bold text-white mb-2">Instant Executive Summaries</h3>
              <p className="text-slate-400 text-xs sm:text-sm leading-relaxed">
                Type on WhatsApp: <span className="text-emerald-400 font-mono italic">"What was our total revenue for Rendang Daging this week?"</span> and get instant math calculations.
              </p>
            </div>
            ...
            <div className="bg-slate-950/40 backdrop-blur-md border border-white/5 hover:border-emerald-500/20 p-6 rounded-2xl transition-all duration-300 group hover:-translate-y-1">
              ...
              <h3 className="text-lg font-bold text-white mb-2">Automated Nightly Reports</h3>
              <p className="text-slate-400 text-xs sm:text-sm leading-relaxed">
                PulseAI will push a clean business performance summary directly to the Owner's WhatsApp every day at 9 PM.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-xl bg-slate-950/60 backdrop-blur-lg border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-2xl">
          <h3 className="text-lg font-bold text-white mb-2">Be the first to experience PulseInternal</h3>
...
export default PulseInternalPage;
